...
import {adaptiveHeading,angleDifference,circularMean,circularSpread,headingFromQuaternion,normalizeHeading} from './compassMath';
...
export default function CompassTools(){
 ...
 function consumeHeading(raw,{source,ios=false,accuracy=NaN}){
  ...
  if(!sourceRef.current){sourceRef.current=source;setSensorSource(source);setNeedsPermission(false)}
 ...
 return <>{open&&<aside className="compassWidget" aria-label="Brújula"><header><div><small>ENCÚMBRATE · ORIENTACIÓN</small><strong>{heading===null?'BRÚJULA':`${String(degrees).padStart(3,'0')}° · ${direction}`}</strong></div><button onClick={()=>setOpen(false)} aria-label="Cerrar brújula">×</button></header><div className="compassDial"><div className="compassTicks"/><div className="compassRose" style={{transform:`rotate(${-heading||0}deg)`}}><b className="north">N</b><b className="east">E</b><b className="south">S</b><b className="west">O</b><i/></div><span className="compassIndex">▲</span><span className="compassHub"/></div><div className={`compassStatus ${calibration}`}><i/><span>{heading!==null?(calibration==='stable'?'Norte estabilizado':'Calibrando…'):needsPermission?'Sensor detenido':'Buscando norte…'}{sensorSource?` · ${sensorSource}`:''}</span></div>{heading===null&&<button className="compassActivate" onClick={activate}>Activar brújula</button>}{error&&<p>{error}</p>}<button className="compassLearn" onClick={()=>{setCourseStep(0);setCourse(true)}} aria-label="Aprender a usar la brújula">?</button></aside>}{course&&<CompassCourse step={courseStep} setStep={setCourseStep} close={()=>setCourse(false)}/>}</>
}
