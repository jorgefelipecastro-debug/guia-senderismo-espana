...
function localResult(answers){
  ...
  if(score>=24 && safetyTotal>=14 && criticalOk && Number(answers.firstAid)>=2 && Number(answers.compass)>=2) level='avanzado+';
  return {score,level};
}
...
export default function HikerAssessment({user,onComplete}){
  ...
  async function next(){if(answers[q.id]===undefined)return; if(step<questions.length-1){setStep(s=>s+1);return;} setBusy(true);setError(''); const storedAnswers=Object.fromEntries(questions.map((question,index)=>[`q${index+1}`,answers[question.id]])); const {data,error}=await supabase.rpc('submit_hiker_assessment',{p_answers:storedAnswers,p_version:1}); setBusy(false); if(error){setError('No hemos podido guardar el test. Inténtalo de nuevo.');return;} setResult(data||calculated);}
  if(result){const level=(result.hiking_level||result.level||calculated.level);return <div className="assessmentOverlay"><section className="assessmentCard resultCard"><div className="assessmentMark">✓</div><small>TEST COMPLETADO</small><h1>Nivel {level}</h1><p>{level==='principiante'?'Empezaremos con rutas accesibles, bien señalizadas y con más apoyo de preparación y seguridad.':level==='intermedio'?'Tienes una buena base. Podremos proponerte rutas de dificultad media y ayudarte a ganar autonomía.':'Tienes experiencia y una base sólida de autonomía, orientación y seguridad. Podremos proponerte retos más exigentes.'}</p><div className="assessmentResultGrid"><span>Puntuación <b>{result.assessment_score ?? result.score ?? calculated.score}</b></span><span>Tu nivel podrá evolucionar con tus <b>Logros</b></span></div><button onClick={()=>onComplete?.(level)}>Entrar en Encúmbrate</button></section></div>}
