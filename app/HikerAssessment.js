...
export default function HikerAssessment({user,onComplete}){
  ...
  async function next(){if(answers[q.id]===undefined)return; if(step<questions.length-1){setStep(s=>s+1);return;} setBusy(true);setError(''); const {data,error}=await supabase.rpc('submit_hiker_assessment',{p_answers:answers,p_version:1}); setBusy(false); if(error){setError('No hemos podido guardar el test. Inténtalo de nuevo.');return;} setResult(data||calculated);}
