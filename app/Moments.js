...
export default function Moments({ user, close }) {
  const [items, setItems] = useState([]), [pending, setPending] = useState([]), [routes, setRoutes] = useState([]), [album, setAlbum] = useState(""), [busy, setBusy] = useState(false), [message, setMessage] = useState(""), [selected, setSelected] = useState(null), cameraPhoto = useRef(null), cameraVideo = useRef(null), gallery = useRef(null);
  ...
  async function load() {
    const [media, activity] = await Promise.all([supabase.from("moments").select("*").eq("user_id", user.id).order("captured_at", { ascending: false }), supabase.from("route_activities").select("id,route_name,distance_km,duration_seconds,ended_at").eq("user_id", user.id).order("started_at", { ascending: false }).limit(50)]);
    if (media.error) return setMessage("Momentos estará disponible cuando se aplique su actualización de seguridad.");
    ...
    setItems(signed); setRoutes(activity.data || []);
  }
