...
export default function ProfilePassport({user,avatar,badge,onClose,onEditAvatar}){
 const[loading,setLoading]=useState(true),[error,setError]=useState(''),[profile,setProfile]=useState(null),[alias,setAlias]=useState(''),[activities,setActivities]=useState([]),[activityDuration,setActivityDuration]=useState(0),[achievements,setAchievements]=useState([]),[submissions,setSubmissions]=useState([]);
 useEffect(()=>{let active=true;async function load(){setLoading(true);setError('');const results=await Promise.all([
  ...
  supabase.from('route_activities').select('id,route_name,route_difficulty,status,distance_km,elevation_gain_m,duration_seconds,trophy_earned,ended_at,created_at').eq('user_id',user.id).order('created_at',{ascending:false}).order('id',{ascending:false}).range(0,19),
  ...
  supabase.rpc('my_route_activity_duration_seconds')
 ]);if(!active)return;setProfile(results[0].data||{});setAlias(results[1].data?.alias||'');setActivities(results[2].data||[]);setAchievements(results[3].data||[]);setSubmissions(results[4].data||[]);setActivityDuration(Number(results[5].data||0));if(results.some(result=>result.error))setError('Algunos datos no se han podido actualizar. Mostramos la información disponible.');setLoading(false)}load();return()=>{active=false}},[user.id]);
 const completed=useMemo(()=>activities.filter(item=>item.status==='completed'&&item.trophy_earned),[activities]),hours=activityDuration/3600,xp=Number(profile?.progression_xp||0),mountain=progressionFor(xp,{expertUnlocked:profile?.accredited_level==='experto',approvedRoutes:Number(profile?.approved_route_contributions||0)}),next=mountain.next,progress=Math.round(mountain.progress),name=(user.user_metadata?.name||alias||'Senderista').trim(),memberSince=date(user.created_at);
