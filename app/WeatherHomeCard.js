...
export default function WeatherHomeCard({data,place,loading,error,onOpen,onRefresh}){
 ...
 const name=data?.municipality||place?.name||'Tu próxima aventura';
 return <section className={`weatherHome theme-${theme}`} aria-label="El tiempo en Inicio">
  <div className="weatherHomeTop"><button onClick={onOpen} className="weatherLocation" aria-label={`Elegir ubicación. ${name}`}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 21s7-7 7-12A7 7 0 0 0 5 9c0 5 7 12 7 12Z"/><circle cx="12" cy="9" r="2.4"/></svg><span>{name}</span><span aria-hidden="true">⌄</span></button><button className="weatherRefresh" onClick={onRefresh} disabled={loading||!data} aria-label="Actualizar el tiempo"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 10a8 8 0 1 0-2 8M20 4v6h-6"/></svg></button></div>
  <button className="weatherHomeMain" onClick={onOpen} aria-label="Abrir previsión meteorológica completa">
   <div className="weatherClock"><small>{selected?'PREVISIÓN POR HORAS':'EL TIEMPO · AEMET'}</small><strong>{selected?selected.time.slice(11,16):parts?`${parts.hour}:${parts.minute}`:'—:—'}</strong><span>{selected?`${selected.time.slice(8,10)}/${selected.time.slice(5,7)} · Hora local`:now?new Intl.DateTimeFormat('es-ES',{timeZone:timezone,weekday:'short',day:'numeric',month:'short'}).format(now):'Prepara tu salida'}</span></div>
   <WeatherScene code={forecast?.code} night={night}/>
   <div className="weatherHomeReading"><div><strong>{value(forecast?.temperature,'°')}<span>C</span></strong><p>{data?condition:loading?'Consultando el cielo…':'Elige tu localidad'}</p></div><span className="weatherRange">{day?`Máx. ${value(day.max,'°')} · Mín. ${value(day.min,'°')}`:'Sol, lluvia y viento antes de salir'}</span></div>
  </button>
