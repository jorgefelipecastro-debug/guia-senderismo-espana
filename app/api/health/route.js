...
export async function GET(request){
 ...
 try{
  ...
  return Response.json({status:'healthy',database:'healthy',latencyMs:Date.now()-started,release:process.env.RAILWAY_GIT_COMMIT_SHA?.slice(0,12)||process.env.VERCEL_GIT_COMMIT_SHA?.slice(0,12)||process.env.GITHUB_SHA?.slice(0,12)||'local'},{headers:{'Cache-Control':'no-store','X-Request-Id':requestId}});
 }catch(error){
