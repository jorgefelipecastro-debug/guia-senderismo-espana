...
async function osmApiGeometry(relationId) {
  ...
  const data = await response.json(),
    nodes = new Map(),
    ways = new Map(),
    relation = (data.elements || []).find(
      (item) =>
        item.type === "relation" && String(item.id) === String(relationId),
    );
  ...
  for (const member of relation?.members || []) {
    ...
    let line = (ways.get(member.ref) || [])
      .map((id) => nodes.get(id))
      .filter(Boolean);
    if (line.length < 2) continue;
    if (member.role === "backward" || member.role === "reverse")
      line = line.reverse();
    if (result.length) {
      const previous = result.at(-1),
        first = line[0],
        last = line.at(-1);
      if (haversine(previous, last) < haversine(previous, first))
        line = line.reverse();
    }
...
async function storedPoints(id) {
  ...
  if (url && key) {
    const client = createClient(url, key, {
      ...
      { data, error } = await client.rpc("get_public_hiking_route_trace", {
        p_route_id: id,
      });
    ...
    if (error) throw error;
    route = data
      ? { trace_points: data.raw_tags?.trace_points, source: data.source }
      : null;
  }
  ...
  return points.length > 1
    ? {
        points,
        source: route.source === "fedamon" ? "FAM" : "Catálogo oficial",
      }
    : null;
}
export async function GET(request) {
  const raw = request.nextUrl.searchParams.get("id") || "",
    match = raw.match(/^osm-relation-(\d+)$/);
  ...
  try {
    const stored = match ? null : await storedPoints(raw);
    if (stored) {
      let kilometres = 0;
      for (let index = 1; index < stored.points.length; index++)
        kilometres += haversine(stored.points[index - 1], stored.points[index]);
      return NextResponse.json(
        {
          points: sample(stored.points),
  ...
  const relationId = match?.[1] || OFFICIAL_OSM_FALLBACKS[raw];
  if (!relationId)
    return NextResponse.json(
      { error: "Esta fuente oficial no publica todavía un trazado navegable." },
      { status: 503 },
    );
  const query = `[out:json][timeout:20];relation(${relationId});out geom;`;
  for (const endpoint of ENDPOINTS) {
    try {
      ...
      if (!response.ok) throw new Error();
      const data = await response.json(),
        points = collectGeometry(data.elements?.[0] || {});
      if (points.length < 2) throw new Error();
      let kilometres = 0;
      for (let index = 1; index < points.length; index++)
        kilometres += haversine(points[index - 1], points[index]);
      return NextResponse.json(
        {
  ...
  try {
    const points = await osmApiGeometry(relationId);
    let kilometres = 0;
    for (let index = 1; index < points.length; index++)
      kilometres += haversine(points[index - 1], points[index]);
    return NextResponse.json(
