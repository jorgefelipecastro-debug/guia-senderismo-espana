...
export function normalizeNationalRoute(element, region, seenAt) {
  ...
  const ascent = validNonNegative(curated?.ascentM ?? numberFrom(tags.ascent || tags['ascent:total'] || tags['ele:gain'] || tags['incline:up']));
  ...
  const duration = validPositive(curated?.durationMinutes ?? durationMinutes(curated?.duration ? { duration: curated.duration } : tags));
