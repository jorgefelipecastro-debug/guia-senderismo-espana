...
export async function fetchRegionRoutes(areaId, regionCode = '') {
  ...
  try {
    ...
    if (!cells) throw error;

    const byId = new Map();
    for (const cell of cells) {
      const cellQuery = `[out:json][timeout:60];area(${areaId})->.searchArea;relation["route"="hiking"](area.searchArea)(${cell.join(',')});out tags center qt;`;
      const payload = await fetchOverpassQuery(cellQuery);
