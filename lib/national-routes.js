import { findCuratedRoute } from './route-curation.js';
import { classifyHikingRoute } from './route-classification.js';
...
// Jaén puede saturar los servidores públicos cuando se consulta toda la
// provincia de una vez. Cada celda conserva el filtro administrativo exacto.
const REGION_QUERY_CELLS = {
  jaen: [
    [37.31, -4.38, 37.95, -3.85],
    [37.31, -3.85, 37.95, -3.32],
    [37.31, -3.32, 37.95, -2.79],
    [37.31, -2.79, 37.95, -2.25],
    [37.95, -4.38, 38.60, -3.85],
    [37.95, -3.85, 38.60, -3.32],
    [37.95, -3.32, 38.60, -2.79],
    [37.95, -2.79, 38.60, -2.25],
  ],
};
...
function distanceFrom(tags) {
  ...
  const kilometres = /\bm\b/i.test(String(raw)) && !/km/i.test(String(raw)) ? amount / 1000 : amount;
  return Number.isFinite(kilometres) && kilometres > 0 ? kilometres : null;
}
...
export async function resolveRegionArea(region) {
  ...
  const relation = results.find(item => item.osm_type === 'relation' && ['province', 'state', 'county'].includes(item.addresstype))
    || results.find(item => item.osm_type === 'relation' && item.type === 'administrative')
    || results.find(item => item.osm_type === 'relation');
...
async function fetchOverpassQuery(query, requestTimeoutMs = 55000) {
  let lastError;
...
export async function fetchRegionRoutes(areaId, regionCode = '') {
  const cells = REGION_QUERY_CELLS[regionCode];
  if (cells) {
    // The full Jaén request has exceeded the host's five-minute limit.
    // Four bounded cells at a time keep the complete, deduplicated result
    // within that limit; never publish an incomplete set of cells.
    const byId = new Map();
    for (let offset = 0; offset < cells.length; offset += 4) {
      const responses = await Promise.all(cells.slice(offset, offset + 4).map(cell => {
        const cellQuery = `[out:json][timeout:35];area(${areaId})->.searchArea;relation["route"="hiking"](area.searchArea)(${cell.join(',')});out tags center qt;`;
        return fetchOverpassQuery(cellQuery, 35000);
      }));
      for (const payload of responses) {
        for (const element of payload.elements || []) byId.set(element.id, element);
      }
    }
    return { elements: [...byId.values()] };
  }
  const query = `[out:json][timeout:90];area(${areaId})->.searchArea;relation["route"="hiking"](area.searchArea);out tags center qt;`;
...
export function normalizeNationalRoute(element, region, seenAt) {
  ...
  const distance = validPositive(curated?.distanceKm ?? distanceFrom(tags));
  const ascent = validNonNegativeInteger(curated?.ascentM ?? numberFrom(tags.ascent || tags['ascent:total'] || tags['ele:gain'] || tags['incline:up']));
  const maxAltitude = validInteger(curated?.maxAltitudeM ?? numberFrom(tags.maxele || tags['ele:max'] || tags.max_altitude || tags.ele));
  const minAltitude = validInteger(curated?.minAltitudeM ?? numberFrom(tags.minele || tags['ele:min'] || tags.min_altitude));
  const duration = validPositiveInteger(curated?.durationMinutes ?? durationMinutes(curated?.duration ? { duration: curated.duration } : tags));
  ...
  if (minAltitude === null) incomplete.push('min_altitude_m');
  ...
  return {
    ...
    level: classifyHikingRoute(distance, ascent), distance_km: distance, ascent_m: ascent,
    ...
    // Fresh relations stay hidden until the geometry verifier saves their line.
    // The database trigger preserves already verified relations on refresh.
    trace_available: false, published: false,
    enrichment_status: incomplete.length ? 'partial' : 'complete',
    incomplete_fields: incomplete, raw_tags: tags,
