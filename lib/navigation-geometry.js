...
export function gpsFixFresh(fix, now = Date.now(), maxAgeMs = 8000) {
  const at = timestampMs(fix),
    current = Number(now),
    limit = Math.max(1000, Number(maxAgeMs) || 8000);
  if (at === null || !Number.isFinite(current)) return false;
  const age = current - at;
  // Pequeñas diferencias entre el reloj del proveedor y el del navegador son normales.
  return age >= -5000 && age <= limit;
}
...
export function plausibleGpsTransition(
  ...
  if (previousAt !== null && nextAt !== null && nextAt < previousAt) return false;
