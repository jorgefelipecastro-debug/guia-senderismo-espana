...
export async function operationalFetch(path, options = {}, fetcher = fetch) {
  const normalized = String(path || "").startsWith("/")
    ? String(path)
    : `/${String(path || "")}`;
  // Track downloads must use the deployment that performs the catalog's
  // verification; the Railway backend currently has an invalid Supabase key.
  if (normalized.startsWith('/api/routes/track?'))
