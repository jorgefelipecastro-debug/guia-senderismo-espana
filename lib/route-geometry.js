...
// OSM relation members are not necessarily listed in walking order. A route
// qualifies for turn-by-turn navigation only if all its ways form one
// continuous walk. Euler's traversal may reverse ways, but never invents a
// connection between two distinct coordinates.
export function continuousRouteLine(segments) {
  const lines = (segments || []).map(cleanLine).filter(line => line.length > 1);
  if (!lines.length) return null;
  const key = point => `${point.lat},${point.lon}`;
  const edges = lines.map(points => ({ points, start: key(points[0]), end: key(points.at(-1)) }));
  const adjacent = new Map();
  for (let index = 0; index < edges.length; index++) {
    for (const vertex of [edges[index].start, edges[index].end]) {
      if (!adjacent.has(vertex)) adjacent.set(vertex, []);
      adjacent.get(vertex).push(index);
    }
  }
  const odd = [...adjacent].filter(([, incident]) => incident.length % 2).map(([vertex]) => vertex);
  if (odd.length !== 0 && odd.length !== 2) return null;
  const origin = lines[0][0];
  const start = odd.length ? odd.reduce((best, vertex) => {
    const [lat, lon] = vertex.split(',').map(Number);
    const [bestLat, bestLon] = best.split(',').map(Number);
    return haversineKm(origin, { lat, lon }) < haversineKm(origin, { lat: bestLat, lon: bestLon }) ? vertex : best;
  }) : key(origin);
  const used = new Uint8Array(edges.length), stack = [{ vertex: start, incoming: null }], reversed = [];
  let traversed = 0;
  while (stack.length) {
    const vertex = stack.at(-1).vertex, incident = adjacent.get(vertex);
    while (incident?.length && used[incident.at(-1)]) incident.pop();
    if (!incident?.length) {
      const step = stack.pop();
      if (step.incoming) reversed.push(step.incoming);
      continue;
    }
    const edgeIndex = incident.pop(), edge = edges[edgeIndex];
    if (used[edgeIndex]) continue;
    used[edgeIndex] = 1;
    traversed++;
    const forward = edge.start === vertex;
    stack.push({ vertex: forward ? edge.end : edge.start,
      incoming: forward ? edge.points : edge.points.slice().reverse() });
  }
  if (traversed !== edges.length) return null;
  const walk = [];
  for (const line of reversed.reverse()) {
    if (walk.length && key(walk.at(-1)) !== key(line[0])) return null;
    walk.push(...(walk.length ? line.slice(1) : line));
  }
  return walk;
}

// A geometry simplifier may skip several real points on a long straight way.
// Restore original vertices wherever that would create a >2 km navigation
// jump, while retaining the simplifier's turns and endpoints.
export function sampleContinuousLine(line, maxPoints = 2000) {
  const selected = simplifyTrack(line, maxPoints);
  if (selected.length === line.length) return selected;
  const indexes = new Map(line.map((point, index) => [point, index]));
  const output = [selected[0]];
  function bridge(start, end) {
    if (haversineKm(line[start], line[end]) <= 2) {
      output.push(line[end]);
      return;
    }
    if (end - start < 2) throw new Error('SOURCE_GEOMETRY_GAP');
  ...
  try {
    for (let index = 1; index < selected.length; index++) {
      bridge(indexes.get(selected[index - 1]), indexes.get(selected[index]));
      if (output.length > 20000) return null;
    }
  } catch { return null; }
  return output;
}
...
export function navigableSegments(segments, maxPoints = 2000) {
  ...
  const line = continuousRouteLine(clean);
  if (!line || routeDistanceKm([line]) < 0.1 ||
      line.some((point, index) => index > 0 && haversineKm(line[index - 1], point) > 2)) return null;
  const sampled = sampleContinuousLine(line, maxPoints);
  return sampled ? [sampled] : null;
}
...
export async function osmRelationGeometry(relationId, fetcher = fetch) {
  ...
  const segments = navigableSegments(relationSegments(await response.json(), relationId));
  if (!segments) throw new Error('OpenStreetMap route geometry unavailable');
