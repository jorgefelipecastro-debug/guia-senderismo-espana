...
test('upgrade purges owned legacy caches and preserves other apps', async()=>{
  const h=harness();
  for(const name of ['encumbrate-v13','encumbrate-v12','cumbre-v1','allzone-v1','encumbrate-public-v20']) await h.caches.open(name);
  await h.dispatch('activate');
  assert.deepEqual(await h.caches.keys(), ['allzone-v1','encumbrate-public-v20']); assert.equal(h.claimed,true);
});
...
test('private shell does not block activation and is never cached',async()=>{
  ...
  await assert.rejects(h.dispatch('install'));assert.equal(h.stores.get('encumbrate-public-v20').size,0);
});
...
test('asset limit preserves the anonymous shell',async()=>{
  ...
  const c=await h.caches.open('encumbrate-public-v20');assert.equal((await c.keys()).length,171);assert.ok(await c.match('/'));
});
