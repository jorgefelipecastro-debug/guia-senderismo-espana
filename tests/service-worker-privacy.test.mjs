...
test('upgrade purges owned legacy caches and preserves other apps', async()=>{
  const h=harness();
  for(const name of ['encumbrate-v13','encumbrate-v12','cumbre-v1','allzone-v1','encumbrate-public-v23']) await h.caches.open(name);
  await h.dispatch('activate');
  assert.deepEqual(await h.caches.keys(), ['allzone-v1','encumbrate-public-v23']); assert.equal(h.claimed,true);
});
...
test('private shell does not block activation and is never cached',async()=>{
  ...
  await assert.rejects(h.dispatch('install'));assert.equal(h.stores.get('encumbrate-public-v23').size,0);
});
...
test('asset limit preserves the anonymous shell',async()=>{
  ...
  const c=await h.caches.open('encumbrate-public-v23');assert.equal((await c.keys()).length,171);assert.ok(await c.match('/'));
});
...
test('cold offline navigation opens a self-contained viewer with all its modules cached',async()=>{
 ...
 for(const path of ['/offline/viewer.mjs','/offline/maps.mjs','/offline/mosaic.mjs','/offline/nav.mjs','/offline/tile-map.mjs']) assert.equal((await h.dispatch('fetch',req(path))).status,200);
